...
-- Drop and recreate the function with lead preservation logic
...
BEGIN
  ...
  IF v_existing_lead_user_id IS NOT NULL THEN
    INSERT INTO public.assignment_participants (assignment_id, user_id, role, chosen_by_admin, chosen_at)
    VALUES (p_assignment_id, v_existing_lead_user_id, 'lead', true, COALESCE(
      (SELECT chosen_at FROM public.assignment_participants 
       WHERE assignment_id = p_assignment_id AND user_id = v_existing_lead_user_id),
      now()
    ))
    ON CONFLICT (assignment_id, user_id) 
    DO UPDATE SET 
      role = 'lead'  -- Ensure role stays as lead
    WHERE public.assignment_participants.assignment_id = p_assignment_id 
      AND public.assignment_participants.user_id = v_existing_lead_user_id;
  END IF;
