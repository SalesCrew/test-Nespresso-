-- Buddy Tag System Database Migration
-- This migration adds full buddy tag support to the database
-- Safe to re-run: uses IF NOT EXISTS and proper enum handling

-- 1) Add 'buddy_tag' status to assignment_status enum (split into steps for PostgreSQL safety)
-- First, add the new value (must be in separate transaction)
ALTER TYPE assignment_status ADD VALUE IF NOT EXISTS 'buddy_tag';

-- 2) Add buddy_name field to assignments table to store the buddy's name
ALTER TABLE public.assignments 
ADD COLUMN IF NOT EXISTS buddy_name text;

-- 3) Add buddy_user_id to store reference to the buddy user
ALTER TABLE public.assignments 
ADD COLUMN IF NOT EXISTS buddy_user_id uuid REFERENCES auth.users(id) ON DELETE SET NULL;

-- 4) Update assignment_invitations to handle buddy tag auto-acceptance
-- When a buddy tag is submitted, it should automatically become 'accepted' and create participant

-- 5) Create function to handle buddy tag acceptance
...
-- 6) Create trigger to automatically handle buddy tag acceptance
...
-- 7) Update the user_assignment_processes view to exclude buddy tags from normal flow
...
-- 10) Create view for admin to see assignments with buddy information
